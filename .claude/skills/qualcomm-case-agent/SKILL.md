...
allowed-tools: Bash(qcase:*), Bash(agent-browser:*), Bash(npx agent-browser:*), Bash(node:*), Bash(powershell:*), PowerShell, Read, Write, Glob
---
...
> **`qcase` CLI.** The skill's scripts are packaged as a `qcase` command (install once:
> `cd .claude/skills/qualcomm-case-agent/scripts && npm link`). SKILL.md calls verbs
> (`qcase intake`, `qcase scrape`, `qcase render`, `qcase script <name>`, `qcase chrome`,
> `qcase login`, `qcase capture-pw`) instead of long `node "<path>"` lines. Full verb
> table + install: `references\setup.md`.
...
| Browser | **real Google Chrome** on CDP `9222` via `qcase chrome` |
...
| CLI | `qcase` (from skill dir `scripts\`): `intake` (validate code + prep dirs) · `chrome` · `login` · `capture-pw` · `script readiness`\|`script extract` (browser-eval payloads, piped to `agent-browser eval --stdin`) · `scrape` (finalizer; `--merge` = incremental update run) · `render` |
| Enrich skill | `qualcomm-enrich` — standalone analyst pass (no browser, no re-scrape); shares the same `qcase render` |
| References | skill dir `references\`: `login-flow.md`, `extraction.md`, `workflow.md`, `consumer-guide.md`, `recovery.md` (Recovery 0/1/2), `setup.md` (install + troubleshooting) |
...
qcase script readiness | agent-browser eval --stdin   # re-probe after click
...
## Recovery 0 / 1 / 2 — reactive branches

When a phase reports a failure state, jump to the matching recovery in **`referencesecovery.md`**:

- **Recovery 0 — Chrome/CDP** (`agent-browser open` errors/times out): reset the daemon, `qcase chrome`, re-attach, retry PHASE 1 once.
- **Recovery 1 — Auth** (`state=AUTH` / bounced to `account.qualcomm.com`): profile auto-fill → `qcase login` → email OTP. Full flow also in `login-flow.md`.
- **Recovery 2 — Empty/Stuck** (`state=BLANK`, or `LOADING` after the 12s ceiling): re-confirm host, reload the SAME url once, re-poll. Runs once, then STOP.
...
3. Make the case folder, then run the bundled extractor via `--stdin` (multi-line JS reaches the browser
   ...
   ```
   If the live DOM differs and fields come back empty, edit `scripts/extract_case.js` in place (it is the
   canonical extractor, not a throwaway; `qcase script extract` emits it verbatim). Header fields
   (title/status/priority) aren't on the Feed view — pass them to `qcase scrape` as flags (next step).
   **Do NOT Read+Edit `case.raw.json` to backfill
...
4. Sanity-check, then finalize (rejects 0 comments → assert count → backfill header flags → SHA-256
   ...
   qcase scrape <CASE_CODE> "data/cases/<CODE>/case.raw.json" --title "<TITLE>" --status "<STATUS>" --priority "<PRIORITY>"
   ```
   ...
   **Update run — finalize with `--merge` instead.** The raw file is a PARTIAL capture (new posts
   ...
   ```bash
   qcase scrape <CASE_CODE> "data/cases/<CODE>/case.raw.json" --merge --status "<STATUS>" --priority "<PRIORITY>"
   ```
...
**Delegation:** same work as standalone `qualcomm-enrich` skill. Hand off to it or run inline — both use the same schema and `qcase render`.
...
All artifacts go in the case folder `data\cases\<CODE>\` (created by `qcase scrape` in PHASE 2).
...
## Setup, other harnesses & troubleshooting

See **`references\setup.md`** for: the `qcase` CLI install (`npm link`) + verb table,
new-machine setup, running under Cline/VS Code, and the Windows gotchas
(`os error 10060` IPv6 mismatch, "input redirection", PowerShell-in-Bash).
