...
const m = {
  ...
  ...(await import(new URL('../.claude/skills/qcomm/scripts/finalize_completeness.mjs', import.meta.url))),
  ...
  ...(await import(new URL('../.claude/skills/qcomm/scripts/finalize_description.mjs', import.meta.url))),
  ...(await import(new URL('../.claude/skills/qcomm/scripts/finalize_header.mjs', import.meta.url))),
  ...(await import(new URL('../.claude/skills/qcomm/scripts/finalize_case.mjs', import.meta.url))),
};
...
describe('finalize (child process)', () => {
  ...
  // raw (title/severity/etc. — see mergeDetailFields), but an update run here
  // only "filled blanks" for those fields. A case renamed in the portal after
  ...
  it('refreshes title/severity from a successful Detail-tab re-scrape on an update run', () => {
    const root = fixture();
    runFinalize(root, RAW); // first capture: title 'NR SA attach failure', no severity
    const partial = {
      ...RAW,
      title: 'NR SA attach failure — RENAMED',
      severity: 'S1',
      detailFields: ['title', 'severity'],
      displayedCommentCount: 3,
      comments: [comment('Carol', 'log attached', { timestamp: '1 hour ago' }), comment('Alice', 'RRC reject on n78', { timestamp: '2 days ago' })],
    };
    const { exit } = runFinalize(root, partial, ['--merge', '--status', 'Closed']);
    assert.equal(exit, m.EXIT.OK);
    const saved = JSON.parse(readFileSync(casePath(root), 'utf8'));
    assert.equal(saved.title, 'NR SA attach failure — RENAMED', 'live Detail-tab title must win on an update run, not the frozen first-capture title');
    assert.equal(saved.severity, 'S1');
  });
  ...
  // no Severity picklist) stays out of detailFields even though `raw[k]` still
  // holds whatever the Feed-tab region of the page happened to match — documented
  // in run_case.mjs's mergeDetailFields docstring as "can stumble onto a
  ...
  it('does not let Feed-tab noise overwrite a cached field the Detail tab did not actually re-render', () => {
    const root = fixture();
    runFinalize(root, { ...RAW, product: 'SM8650' });
    const partial = {
      ...RAW,
      product: 'garbage-from-feed', // Detail tab has no Product field on this case; raw.product is stale Feed-tab noise
      detailFields: ['title'], // Detail tab rendered Subject, but not Product, this run
      displayedCommentCount: 3,
      comments: [comment('Carol', 'log attached', { timestamp: '1 hour ago' }), comment('Alice', 'RRC reject on n78', { timestamp: '2 days ago' })],
    };
    const { exit } = runFinalize(root, partial, ['--merge']);
    assert.equal(exit, m.EXIT.OK);
    const saved = JSON.parse(readFileSync(casePath(root), 'utf8'));
    assert.equal(saved.product, 'SM8650', 'a field the Detail tab did not re-render must never be clobbered by Feed-tab noise');
  });
  ...
  // Detail tab genuinely doesn't render for this case (e.g. no Severity picklist)
  // stays at whatever the Feed-tab region of `raw` happened to hold — documented
  // in run_case.mjs's mergeDetailFields docstring as "can still stumble onto a
  // non-empty value ... wrong DOM region". Trusting detailExtracted alone lets
  // that Feed-tab noise overwrite a good cached value.
  it('keeps the cached title when the Detail-tab re-scrape failed (thin/degraded capture)', () => {
    const root = fixture();
    runFinalize(root, RAW);
    const partial = {
      ...RAW,
      title: '', // Feed-tab alone left title blank; Detail switch failed
      displayedCommentCount: 3,
      comments: [comment('Carol', 'log attached', { timestamp: '1 hour ago' }), comment('Alice', 'RRC reject on n78', { timestamp: '2 days ago' })],
    };
    const { exit } = runFinalize(root, partial, ['--merge']);
    assert.equal(exit, m.EXIT.OK);
    const saved = JSON.parse(readFileSync(casePath(root), 'utf8'));
    assert.equal(saved.title, 'NR SA attach failure', 'a degraded capture must never clobber the cached title');
  });
