...
## Acceptance criteria
- [x] Implement `renderDashboardHtml(overviewData)` producing a self-contained HTML file saved to `data/cases/dashboard.html` with zero external CDN dependencies (works 100% offline).
- [x] Dashboard UI features:
...
- [x] Implement `renderCliTable(overviewData, options)` rendering a clean table/markdown to stdout for terminal and AI agent consumption.
- [x] Implement CLI flag `--open` to launch the generated `data/cases/dashboard.html` in the OS default browser.
- [x] Unit & rendering tests in `tests/cases_overview_render.test.mjs`.
