...
```jsonc
{
  ...
  "comments": [                            // NEWEST FIRST, replies grouped under parent, verbatim, never truncated
    { "id": "c9f2a1b7c4d0",                // content id — sha256(author + body prefix), see below
      "timestamp": "…", "author": "…",
      "body": "…", "parentId": null, "analysisLog": [], "attachments": [] }
  ],
...
> Source fingerprint `e4a9054be9f5` over 67 files.
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-agent/scripts/_paths.mjs` | 65 | single source of truth for skill paths (Node / ESM). |
| `.claude/skills/qualcomm-case-agent/scripts/_paths.ps1` | 74 | single source of truth for skill paths (PowerShell). |
| `.claude/skills/qualcomm-case-agent/scripts/browser.mjs` | 270 | thin Node wrapper around the `agent-browser` CLI. |
...
| `.claude/skills/qualcomm-case-agent/scripts/check_collapsed.js` | 126 | Pure read: how many non-anchor posts still end in the "Expand Post" label right now. |
| `.claude/skills/qualcomm-case-agent/scripts/connect_chrome.ps1` | 170 | launch REAL system Chrome detached with a CDP port + dedicated persistent profile, ready for 'agent-browser connect <port>'. |
| `.claude/skills/qualcomm-case-agent/scripts/delete_case.mjs` | 134 | permanent, agent-confirmed removal of one case's local cache. |
...
| `.claude/skills/qualcomm-case-agent/scripts/fast_landing.mjs` | 581 | — |
...
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 645 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 1006 | Persistence post-processor for the AGENT-DRIVEN extraction. |
| `.claude/skills/qualcomm-case-agent/scripts/secret_store.mjs` | 51 | DPAPI secret store for Qualcomm ID password (issue #126). |
| `.claude/skills/qualcomm-case-agent/scripts/switch_tab.js` | 113 | Switches active tab on Salesforce Lightning case page (e.g., between "Detail" and "Feed"). |
...
| `browser.mjs` | `closeCdpClient()` | async fn |  |
...
| `browser.mjs` | `evalFile(scriptPath, vars = {}, opts = {})` | function | Evaluate a page script in the attached tab and return its parsed result. |
| `browser.mjs` | `evalFileViaCdp(cdp, scriptPath, vars = {})` | async fn | Same page-script contract as evalFile (strip comments, wrap in the payload IIFE), but sent straight over an already-open CDP WebSocket via `cdp.eval` instead of shelling out through cmd.exe. |
...
| `browser.mjs` | `click(selector)` | function |  |
| `browser.mjs` | `pdf(path)` | function |  |
| `browser.mjs` | `screenshot(path)` | function | Full-page PNG of the feed exactly as expansion left it — the visual evidence that every "Expand Post" / "More comments" really did get clicked. |
...
| `delete_case.mjs` | `deleteCase(rawCode, dataDir = DATA_DIR)` | function | Delete one case's entire local cache: its directory, its _index.json entry, then resync the qualcomm-case-overview aggregation/dashboard. |
...
| `migrate_case.mjs` | `sanitizeComment(comment, referenceDate = new Date())` | function |  |
| `migrate_case.mjs` | `stripFieldAffordances(s, label = '')` | function |  |
| `migrate_case.mjs` | `migrateCaseData(caseData, options = {})` | function |  |
| `migrate_case.mjs` | `detectFlattenedBodies(caseData)` | function | Detects comment bodies that were flattened at capture time (no internal line breaks despite containing multi-sentence text or log sequences). |
| `migrate_case.mjs` | `migrateCaseJson(jsonPath, options = {})` | function |  |
| `migrate_case.mjs` | `migrateCase` | value |  |
| `migrate_case_detail.mjs` | `SUPPORTED_DETAIL_FLAGS` | value |  |
| `migrate_case_detail.mjs` | `parseDetailFlags(argv)` | function | Parses supported CLI flags from argv into an overrides object. |
| `migrate_case_detail.mjs` | `migrateCaseDetailData(caseData, overrides = {})` | function | Normalizes and upgrades case data with Salesforce Detail metadata. |
| `migrate_case_detail.mjs` | `migrateCaseDetailJson(jsonPath, overrides = {}, options = {})` | function | Migrates a case.json file, re-renders case.md and updates _index.json. |
| `migrate_case_detail.mjs` | `migrateCaseDetail` | value |  |
...
| `scrape_case.mjs` | `EXIT` | value | Exit codes (exported so tests can import) |
| `scrape_case.mjs` | `computeHash(raw)` | function | Hash only the raw, verbatim fields. |
| `scrape_case.mjs` | `findCollapsed(comments, newIds)` | function | Only check comments NEW to this capture — a merge (and a full re-capture of a cache) deliberately leaves OLD posts collapsed (see expand_step.js) and keeps their cached verbatim bodies, so those legitimately still carry the label in the freshly re-extracted DOM. |
| `scrape_case.mjs` | `countAssert(capturedCount, displayedCount)` | function | Completeness gate comparison (Issue #91): The Salesforce Chatter badge ("N Chatter Feed Items") counts only top-level posts, whereas our Feed extractor captures both top-level posts AND nested replies (e.g. |
| `scrape_case.mjs` | `HEADER_KEYS` | value | Header fields the agent already holds in-context from the PHASE 1 search row. |
| `scrape_case.mjs` | `DETAIL_KEYS` | value | Salesforce Lightning Detail tab metadata fields persisted to canonical case.json. |
...
| `scrape_case.mjs` | `migrateIds(cached)` | function | Bring a cached case written with the old positional ids (c1, c2, …) or with raw DOM ids onto content ids. |
| `scrape_case.mjs` | `isBlacklistedTs(s)` | function | Checks if a timestamp string contains Salesforce/Chatter UI tooltip noise. |
| `scrape_case.mjs` | `extractSummary(body)` | function | Extracts a concise 1-2 sentence preview summary from raw comment body, stripping common email greetings/salutations. |
| `scrape_case.mjs` | `synthesizeDescriptionComment(raw)` | function | Synthesizes an initial comment representing the case problem statement from raw case description, if non-empty. |
| `scrape_case.mjs` | `hasDescriptionComment(comments, description)` | function | Checks if a comment matching the case description is already present. |
| `scrape_case.mjs` | `genuineCommentCount(comments, description)` | function | The synthesized description comment is a presentation convenience derived from the Case's description field, not a captured Chatter feed item — the completeness gate must compare against genuine portal comments only. |
| `scrape_case.mjs` | `classifyRole(author, company = '', context = '', body = '')` | function | Classifies author role into 'Qualcomm', 'Customer', or 'System' based on author name, company, and body clues. |
| `scrape_case.mjs` | `parseTimestamp(ts, referenceDate = new Date())` | function | Normalizes and parses various timestamp formats into epoch milliseconds. |
| `scrape_case.mjs` | `isRelativeTimestamp(ts)` | function | Checks if a timestamp string is a relative Chatter format (e.g. |
| `scrape_case.mjs` | `normalizeComment(comment, referenceDate = new Date())` | function | Normalizes relative Chatter timestamp to absolute ISO-8601 string resolved against capture reference date, retaining raw portal string in `rawTimestamp`. |
| `scrape_case.mjs` | `normalizeComments(comments, referenceDate = new Date())` | function | Normalizes all comments in an array. |
| `scrape_case.mjs` | `sortCommentsChronological(comments, referenceDate = new Date())` | function | Sorts comments strictly in chronological order (Oldest -> Newest). |
| `scrape_case.mjs` | `orderCommentsForPresentation(comments)` | function | Final PRESENTATION order for case.json/case.md (supersedes PRD #105-109's strict Oldest -> Newest "Variant A"): newest activity first, with each reply grouped immediately after its parent (both threads and same-thread replies ordered newest-first). |
| `scrape_case.mjs` | `mergeComments(cachedComments, rawComments, referenceDate = new Date())` | function | Merge raw comments not already cached and enforce chronological sorting (Oldest -> Newest). |
| `scrape_case.mjs` | `finalize(caseCode, rawPath, header = {}, merge = false, options = {})` | function | Main |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-summary/scripts/cap.mjs` | 15 | pure character-cap guard for qualcomm-case-summary. |
| `.claude/skills/qualcomm-case-summary/scripts/delta.mjs` | 9 | pure delta computation for qualcomm-case-summary. |
| `.claude/skills/qualcomm-case-summary/scripts/deps.mjs` | 37 | the ONE effects boundary for qualcomm-case-summary. |
| `.claude/skills/qualcomm-case-summary/scripts/merge.mjs` | 31 | pure merge of a new batch of comment summaries into summary.json's shape. |
| `.claude/skills/qualcomm-case-summary/scripts/render_summary.mjs` | 56 | pure summary.md renderer for qualcomm-case-summary. |
| `.claude/skills/qualcomm-case-summary/scripts/run_summary.mjs` | 118 | orchestrator for qualcomm-case-summary. |

Exported API — summary scripts:

| Module | Export | Kind | Contract |
|---|---|---|---|
| `cap.mjs` | `CHAR_CAP` | value |  |
| `cap.mjs` | `applyCharCap(body, cap = CHAR_CAP)` | function |  |
| `cap.mjs` | `applyCharCapToComments(comments, cap = CHAR_CAP)` | function |  |
| `delta.mjs` | `computeDelta(caseComments, summarizedIds)` | function |  |
| `deps.mjs` | `captureCase(code)` | async fn | Invokes qualcomm-case-agent's capture pipeline for one case code and returns its parsed verdict object (the JSON line on stdout), regardless of exit code several non-zero exits (auth-required, not-found, blocked, busy) are expected verdicts, not invocation failures. |
| `merge.mjs` | `mergeSummary(prior, { caseNumber, title, url, priority, product, status, newComments, flow, executive, now })` | function |  |
| `render_summary.mjs` | `renderSummaryMd(summary)` | function |  |
| `run_summary.mjs` | `prepare(code)` | async fn |  |
| `run_summary.mjs` | `finalize(code, { comments: newComments, flow, executive })` | function |  |

#### Overview scripts

| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-overview/scripts/cases_overview.mjs` | 1917 | Aggregation engine, CLI summary table, and offline HTML dashboard for Qualcomm cases. |

Exported API — overview scripts:

| Module | Export | Kind | Contract |
|---|---|---|---|
| `cases_overview.mjs` | `escapeHtml(str)` | function | Escapes HTML characters in string to prevent XSS. |
| `cases_overview.mjs` | `getStatusCategory(status)` | function | Categorizes status for badge colors and filtering. |
| `cases_overview.mjs` | `createCommentSnippet(comment, maxLen = 160)` | function | Normalizes text snippet from a comment (removes duplicate whitespace/newlines, truncates). |
| `cases_overview.mjs` | `extractProductFromTitle(title)` | function | Extracts product code from title if product field is missing or empty. |
| `cases_overview.mjs` | `extractAiSummary(summaryJson)` | function | Extracts formatted AI executive summary from summary.json data. |
| `cases_overview.mjs` | `extractCaseOverview(caseDir, caseNumber = '')` | function | Extracts normalized overview record for a single case directory. |
| `cases_overview.mjs` | `computeStats(cases)` | function | Recomputes overview statistics from an array of cases. |
| `cases_overview.mjs` | `buildOverviewData(casesDir = DEFAULT_CASES_DIR)` | function | Scans all case directories in casesDir and aggregates overview data. |
| `cases_overview.mjs` | `updateCaseOverview(caseNumber, casesDir = DEFAULT_CASES_DIR)` | function | Incrementally updates or inserts a single case record in _overview.json atomically. |
| `cases_overview.mjs` | `renderDashboardHtml(overviewData, outputPath = null)` | function | Generates self-contained, offline HTML Dashboard. |
| `cases_overview.mjs` | `renderCliTable(overviewData, options = {})` | function | Renders formatted terminal summary and case table. |
...
| `cases_overview.mjs` | `applyFilter(overviewData, filter)` | function | Filters overview cases by status string (case-insensitive substring match). |
...
| `tests/cases_overview_data.test.mjs` | 542 | Unit and integration tests for cases_overview data scanning and aggregation engine. |
| `tests/cases_overview_e2e.test.mjs` | 267 | End-to-end integration tests for cases_overview: 1. |
| `tests/cases_overview_render.test.mjs` | 449 | Unit and integration tests for cases_overview rendering (HTML Dashboard & CLI table). |
| `tests/cdp_client.test.mjs` | 283 | Unit tests for Native WebSocket CDP Client (Slice 1). |
| `tests/chronological_sort.test.mjs` | 309 | Unit tests for chronological comment timestamp parsing and Oldest -> Newest sorting. |
| `tests/delete_case.test.mjs` | 151 | Tests for delete_case.mjs — permanent, agent-confirmed case cache removal. |
| `tests/description_first_comment.test.mjs` | 405 | Comprehensive test suite for PRD #53 / Issues #54, #55, #56: Case Description as Initial Comment across Extraction, Ingestion, Merge, Rendering, and Migration. |
| `tests/e2e_landing_smoke.mjs` | 135 | E2E Smoke Test for Fast CDP Landing Engine. |
| `tests/ensure_chrome_port_identity.test.mjs` | 116 | Tests for browser.mjs's CDP-port identity check (issue #104). |
| `tests/ensure_chrome_timing.test.mjs` | 61 | Regression test for fast non-blocking ensureChrome(). |
| `tests/ensure_protocol.test.mjs` | 138 | Unit and integration tests for scripts/ensure_protocol.mjs (Self-Healing Protocol Engine). |
| `tests/extract_case.test.mjs` | 1825 | — |
| `tests/fast_landing.test.mjs` | 996 | — |
| `tests/intake.test.mjs` | 121 | QA coverage for intake.mjs — the skill's INPUT CONTRACT gate. |
| `tests/login_fill.test.mjs` | 138 | — |
| `tests/migrate_case.test.mjs` | 520 | — |
| `tests/migrate_case_detail.test.mjs` | 234 | — |
...
| `tests/protocol_registration.test.mjs` | 178 | Unit & integration tests for Windows Registry protocol registration scripts. |
...
| `tests/qualcomm_case_summary_metadata.test.mjs` | 189 | Tests for qualcomm-case-summary's case-level metadata header + executive summary block. |
| `tests/qualcomm_case_summary_orchestrator.test.mjs` | 259 | Tests for qualcomm-case-summary's orchestrator (prepare/finalize). |
| `tests/qualcomm_case_summary_render.test.mjs` | 149 | Tests for qualcomm-case-summary's summary.md renderer (pure, no mocking needed). |
| `tests/render_case.test.mjs` | 712 | QA coverage for render_case.mjs Tests that render_case.mjs generates ONLY case.md (clean markdown) with Header metadata, Initial Description, and Chronological Timeline of comments, without generating HTML, PDF, txt, or report.md files. |
| `tests/run_case.test.mjs` | 749 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
| `tests/scrape_case.test.mjs` | 1080 | Tests for the finalizer — the module that decides what gets persisted. |
| `tests/screenshot_timeout.test.mjs` | 39 | Regression test for bounded screenshot() timeout. |
| `tests/secret_store.test.mjs` | 145 | Tests for secret_store.mjs (DPAPI secret store, issue #126). |
| `tests/verify_case.test.mjs` | 102 | Tests for the post-capture QA gate. |
...
| File | Lines | Purpose |
|---|---|---|
| `tools/gen_design.mjs` | 219 | regenerate the machine-derived part of docs/DESIGN.md. |
...
| `npm run cases:overview` | `node .claude/skills/qualcomm-case-overview/scripts/cases_overview.mjs` |
| `npm run cases:dashboard` | `node .claude/skills/qualcomm-case-overview/scripts/cases_overview.mjs --open` |
| `npm run setup:protocol` | `powershell -ExecutionPolicy Bypass -File scripts/register_protocol.ps1` |
| `npm run uninstall:protocol` | `powershell -ExecutionPolicy Bypass -File scripts/unregister_protocol.ps1` |
| `npm run postinstall` | `node scripts/ensure_protocol.mjs` |
