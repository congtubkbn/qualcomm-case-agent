...
> Source fingerprint `23c74fe061b7` over 62 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/finalize_case.mjs` | 1002 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `tests/cases_overview_after_finalize.test.mjs` | 69 | Tests for cases_overview.mjs's afterFinalize — the shared overview/dashboard sync seam both finalize_case.mjs and run_summary.mjs call after a case finalizes. |
...
| `tests/delete_case.test.mjs` | 235 | Tests for delete_case.mjs — permanent, agent-confirmed case cache removal. |
...
| `tests/finalize_case.test.mjs` | 1105 | Tests for the finalizer — the module that decides what gets persisted. |
