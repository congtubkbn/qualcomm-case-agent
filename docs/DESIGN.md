...
| `.claude/skills/qualcomm-case-agent/scripts/browser.mjs` | 164 | thin Node wrapper around the `agent-browser` CLI. |
...
| `.claude/skills/qualcomm-case-agent/scripts/expand_step.js` | 93 | PHASE 1.5 (A and B) as ONE browser-side tick, called in a loop from run_case.mjs. |
| `.claude/skills/qualcomm-case-agent/scripts/extract_case.js` | 104 | Default case extractor for PHASE 2. |
| `.claude/skills/qualcomm-case-agent/scripts/find_case_link.js` | 74 | PHASE 1 "click the search result" — done browser-side instead of by the agent. |
...
| `.claude/skills/qualcomm-case-agent/scripts/okta_login.ps1` | 107 | drive Qualcomm Okta identifier-first login through the password step using the DPAPI-stored credential. |
| `.claude/skills/qualcomm-case-agent/scripts/readiness.js` | 62 | PHASE 1 readiness probe. |
| `.claude/skills/qualcomm-case-agent/scripts/readiness.ps1` | 8 | — |
...
| `enrich_local.mjs` | `cfg` | value | Mutable so the CLI flags below can override the env defaults. |
| `enrich_local.mjs` | `extractJson(text)` | function | Pull the first balanced JSON object out of a model response. |
| `enrich_local.mjs` | `normalizeAnalysis(obj)` | function | Coerce a model response into the commentAnalyses schema, or null if unusable. |
| `enrich_local.mjs` | `normalizeCaseLevel(obj)` | function | Coerce a model response into the case-level enrichment fields, or null. |
| `enrich_local.mjs` | `buildCaseFlow(comments, analyses)` | function | Oldest→newest debug narrative, derived from the per-comment roles (no model call needed — the ordering and roles already carry it). |
| `intake.mjs` | `intake(raw)` | function | Validate a case code and prepare its cache folder. |
| `lock.mjs` | `LOCK_PATH` | value |  |
| `lock.mjs` | `acquireLock(path = LOCK_PATH, now = Date.now())` | function | Try to take the capture lock. |
...
| `run_case.mjs` | `isNoUpdate(probe, cached)` | function | Unchanged iff the feed total AND the top post both still match the cache. |
| `scheduler.mjs` | `WATCHLIST_PATH` | value |  |
| `scheduler.mjs` | `RUNS_PATH` | value |  |
| `scheduler.mjs` | `readJson(path, fallback)` | function |  |
| `scheduler.mjs` | `writeJson(path, value)` | function |  |
| `scheduler.mjs` | `loadWatchlist()` | function |  |
| `scheduler.mjs` | `dueCases(watchlist, runs, now = Date.now())` | function | Cases whose own interval (or the global default) has elapsed. |
| `scheduler.mjs` | `runCase(code, watchlist)` | function | Run one case through the full pipeline. |
| `scheduler.mjs` | `sweep({ only = null } = {})` | function |  |
| `scrape_case.mjs` | `EXIT` | value | Exit codes (exported so tests can import) |
| `scrape_case.mjs` | `computeHash(raw)` | function | Hash only the raw, verbatim fields (never enrichment) so re-enriching a case does not change its identity. |
| `scrape_case.mjs` | `countAssert(capturedCount, displayedCount)` | function | captured < displayed => the agent must expand more / re-extract (do NOT persist a partial capture). |
...
Exported API — dashboard:

| Module | Export | Kind | Contract |
|---|---|---|---|
| `server.mjs` | `projectCase(code, caseJson, indexEntry, run)` | function | Small, UI-shaped projection of one cached case — never the whole comment set. |
| `server.mjs` | `createApp()` | function |  |
