...
> Source fingerprint `41bd177edcbf` over 77 files.
...
| `.claude/skills/qcomm/scripts/dashboard_renderer.mjs` | 195 | Assembles the self-contained, offline HTML dashboard for Qualcomm case overview data from its styles, client script, and row/page templating modules. |
| `.claude/skills/qcomm/scripts/dashboard_row.mjs` | 193 | Renders a single case's summary/detail row pair for the dashboard table. |
...
| `dashboard_row.mjs` | `escapeHtml(str)` | function | Escapes HTML characters in string to prevent XSS. |
| `dashboard_row.mjs` | `getStatusCategory(status, ballInCourt)` | function | Categorizes status for badge colors and filtering. |
