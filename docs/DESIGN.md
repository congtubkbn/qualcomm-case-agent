...
> Source fingerprint `8772ba1ae680` over 83 files.
...
| `.claude/skills/qcomm/scripts/cdp_portal_driver.mjs` | 332 | Production PortalDriver: drives the persistent-profile Chrome over CDP. |
...
| `.claude/skills/qcomm/scripts/repeat_until_stable.mjs` | 50 | CDP-agnostic polling primitive (#275): ticks a caller-supplied function until its result stops changing, or gives up after maxRounds. |
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 930 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
...
| `tests/qcomm_repeat_until_stable.test.mjs` | 85 | Unit tests for the CDP-agnostic repeatUntilStable polling primitive (#275). |
