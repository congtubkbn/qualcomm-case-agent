...
> Source fingerprint `f7e9fd039467` over 75 files.
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 385 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
