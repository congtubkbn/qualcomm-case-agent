...
> Source fingerprint `9394f41f84ed` over 81 files.
...
| `.claude/skills/qcomm/scripts/cases_overview.mjs` | 171 | CLI orchestrator: wires the overview store, dashboard renderer, and CLI table renderer together into the `cases_overview` command. |
...
| `.claude/skills/qcomm/scripts/dashboard_client_script.mjs` | 438 | Embedded client-side script for the offline Qualcomm case dashboard (filtering, searching, theme toggle, auto-refresh, modal interactions). |
| `.claude/skills/qcomm/scripts/dashboard_renderer.mjs` | 198 | Assembles the self-contained, offline HTML dashboard for Qualcomm case overview data from its styles, client script, and row/page templating modules. |
| `.claude/skills/qcomm/scripts/dashboard_row.mjs` | 152 | Renders a single case's summary/detail row pair for the dashboard table. |
| `.claude/skills/qcomm/scripts/dashboard_styles.mjs` | 806 | Embedded CSS stylesheet for the offline Qualcomm case dashboard. |
...
| `.claude/skills/qcomm/scripts/overview_store.mjs` | 388 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
...
| `cli_renderer.mjs` | `renderCliTable(overviewData, options = {})` | function | @param {object} overviewData @param {object} [options={}] @returns {string} Formatted CLI output string |
...
| `dashboard_client_script.mjs` | `renderClientScript()` | function | Renders embedded client-side script for filtering, searching, and interactions. |
...
| `dashboard_row.mjs` | `renderCaseRow(c)` | function | Renders the summary row and expandable detail row pair for a single case. |
| `dashboard_styles.mjs` | `renderStyles()` | function | Renders embedded CSS stylesheet for the dashboard. |
...
| `open_qc_case.mjs` | `isValidQualcommUrl(urlString)` | function | @param {string} urlString @returns {boolean} |
| `open_qc_case.mjs` | `parseQcUri(uriString)` | function | Supported patterns: - qc://case/<caseNumber> - qc://<caseNumber> - qc:case/<caseNumber> - qc:<caseNumber> - qc://open?url=<encoded-url> - qc:open?url=<encoded-url> @param {string} uriString @returns {{ type: 'case', caseNumber: string } \| { type: 'url', url: string }} |
...
| `open_qc_case.mjs` | `dispatchQcTarget(targetUrl, options = {})` | async fn | @param {string} targetUrl @param {Object} [options] @param {number} [options.port] @param {string} [options.host] @param {string} [options.profileDir] @param {Function} [options.launchChromeFn] @returns {Promise<{ success: boolean, method: 'cdp' \| 'launch', targetUrl: string, targetId?: string }>} |
| `open_qc_case.mjs` | `openQcCase(rawUri, options = {})` | async fn | @param {string} rawUri @param {Object} [options] @param {boolean} [options.throwOnError=true] @param {number} [options.port] @param {string} [options.casesDir] @param {Function} [options.launchChromeFn] @returns {Promise<{ success: boolean, method?: string, targetUrl?: string, error?: string }>} |
