...
> Source fingerprint `35c39155f474` over 86 files.
...
| `.claude/skills/qcomm/scripts/comment_tree.mjs` | 130 | Sole owner of the Comment Tree shape: the nested `subs:[]` structure that case.json and summary.json both persist (a Comment's replies live in its own `subs` array, not a separate structure — see CONTEXT.md's Comment Tree entry). |
