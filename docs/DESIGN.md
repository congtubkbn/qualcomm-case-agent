...
> Source fingerprint `42b99bad9f26` over 76 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/check_collapsed.js` | 54 | Pure read: how many non-anchor posts still end in the "Expand Post" label right now. |
...
| `.claude/skills/qualcomm-case-agent/scripts/delete_case.mjs` | 131 | permanent, agent-confirmed removal of one case's local cache. |
| `.claude/skills/qualcomm-case-agent/scripts/dom_helpers.js` | 138 | Shared DOM helpers for page scripts (expand_step.js, check_collapsed.js, switch_tab.js, login_fill.js). |
| `.claude/skills/qualcomm-case-agent/scripts/expand_step.js` | 151 | PHASE 1.5 (A and B) as ONE browser-side tick, called in a loop from run_case.mjs. |
...
| `.claude/skills/qualcomm-case-agent/scripts/finalize_case.mjs` | 1001 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `delete_case.mjs` | `deleteCase(rawCode, dataDir = DATA_DIR, options = {})` | function | Delete one case's entire local cache: its directory, its _index.json entry, then resync the qualcomm-case-overview aggregation/dashboard. |
...
| `finalize_case.mjs` | `orderCommentsForPresentation(comments)` | function | Final PRESENTATION order for case.json/case.md (supersedes PRD #105-109's strict Oldest -> Newest "Variant A"): newest activity first, with each reply grouped immediately after its parent (both threads and same-thread replies ordered newest-first). |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-summary/scripts/run_summary.mjs` | 261 | orchestrator for qualcomm-case-summary. |
...
| `run_summary.mjs` | `mergeSummary(prior, { caseNumber, title, url, priority, product, status, newComments, parentIdOf, flow, executive, now })` | function | merge.mjs inline |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-overview/scripts/cases_overview.mjs` | 185 | CLI orchestrator: wires the overview store, dashboard renderer, and CLI table renderer together into the `cases_overview` command. |
| `.claude/skills/qualcomm-case-overview/scripts/cli_renderer.mjs` | 68 | Renders the terminal summary table for Qualcomm case overview data. |
| `.claude/skills/qualcomm-case-overview/scripts/dashboard_renderer.mjs` | 1582 | Renders the self-contained, offline HTML dashboard for Qualcomm case overview data. |
| `.claude/skills/qualcomm-case-overview/scripts/overview_store.mjs` | 506 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
| `.claude/skills/qualcomm-case-overview/scripts/staleness.mjs` | 21 | Shared staleness formatting, used by both the HTML dashboard and CLI table renderers. |

Exported API — overview scripts:

| Module | Export | Kind | Contract |
|---|---|---|---|
| `cases_overview.mjs` | `afterFinalize(caseCode, dataDir, options = {})` | function | Syncs the cases overview and dashboard after a case finalizes (capture or summary). |
...
| `overview_store.mjs` | `acquireOverviewLock(casesDir, opts = {})` | function | Acquires the exclusive lock guarding _overview.json's read-modify-write-rename. |
| `overview_store.mjs` | `releaseOverviewLock(lock)` | function | Releases a lock acquired by acquireOverviewLock. |
...
| `.claude/skills/qualcomm-issue-precedent/scripts/precedent_store.mjs` | 196 | Data/domain module: scans case directories for Reference Cases (a Case whose summary.json.executive.rootCause is a non-empty string), extracts verbatim technical signatures from their rootCause/resolution/comment text, and ranks them against a free-text query by deterministic keyword overlap. |
...
| `precedent_store.mjs` | `searchPrecedents(query, casesDir = DEFAULT_CASES_DIR, limit = 10)` | function | Ranks the Reference Case corpus against a free-text query by deterministic keyword overlap, highest score first (ties broken by ascending case number), truncated to `limit`. |
...
| `tests/cases_overview_after_finalize.test.mjs` | 68 | Tests for cases_overview.mjs's afterFinalize — backwards-compatibility wrapper delegating to syncCaseOverview. |
| `tests/cases_overview_data.test.mjs` | 745 | Unit and integration tests for cases_overview data scanning and aggregation engine. |
| `tests/cases_overview_e2e.test.mjs` | 319 | End-to-end integration tests for cases_overview: 1. |
| `tests/cases_overview_lock.test.mjs` | 127 | Tests for overview_store.mjs's write lock around _overview.json (issue #202): concurrent syncCaseOverview calls for different cases must not last-write-wins clobber each other, and the lock itself must take over a stale holder and give up after a bounded wait instead of hanging forever. |
| `tests/cases_overview_render.test.mjs` | 578 | Unit and integration tests for cases_overview rendering (HTML Dashboard & CLI table). |
| `tests/cdp_client.test.mjs` | 283 | Unit tests for Native WebSocket CDP Client (Slice 1). |
| `tests/chronological_sort.test.mjs` | 325 | Unit tests for chronological comment timestamp parsing and Oldest -> Newest sorting. |
| `tests/credentials_setup.test.mjs` | 76 | — |
| `tests/delete_case.test.mjs` | 253 | Tests for delete_case.mjs — permanent, agent-confirmed case cache removal. |
...
| `tests/dom_helpers.test.mjs` | 209 | Loads dom_helpers.js's exact source into a jsdom context (script execution, not a hand-copied re-implementation) and exercises each shared helper — including the shadow-DOM case behind case 08503838 that motivated this file (see dom_helpers.js). |
...
| `tests/finalize_case.test.mjs` | 1178 | Tests for the finalizer — the module that decides what gets persisted. |
...
| `tests/precedent_report.test.mjs` | 105 | Unit tests for precedent_report.mjs's rendering/persistence — prior art: tests/qualcomm_case_summary_render.test.mjs testing render_summary.mjs's exported functions directly. |
...
| `tests/precedent_store.test.mjs` | 255 | Unit tests for precedent_store's Reference Case filtering, scoring, and signature extraction. |
...
| `tests/qualcomm_case_summary_merge.test.mjs` | 152 | Tests for qualcomm-case-summary's merge logic (pure, no mocking needed). |
...
| `tests/qualcomm_case_summary_orchestrator.test.mjs` | 356 | Tests for qualcomm-case-summary's orchestrator (prepare/finalize). |
...
| `tests/run_case.test.mjs` | 842 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
...
| File | Lines | Purpose |
|---|---|---|
| `tools/gen_design.mjs` | 220 | regenerate the machine-derived part of docs/DESIGN.md. |
