...
> Source fingerprint `32329655ab46` over 79 files.
...
| `.claude/skills/qcomm/scripts/finalize_case.mjs` | 365 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `.claude/skills/qcomm/scripts/portal_driver.mjs` | 86 | the seam between run_case.mjs (orchestration: verdicts, retries, screenshots-per-failure-mode) and however a case actually gets read off the portal. |
...
| `.claude/skills/qcomm/scripts/run_case.mjs` | 553 | the whole capture pipeline as ONE deterministic command. |
...
| `tests/qcomm_run_case.test.mjs` | 1080 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
