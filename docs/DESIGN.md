...
> Source fingerprint `a2f929ec5c99` over 29 files.
...
| File | Lines | Purpose |
|---|---|---|
| `web/cli_run.mjs` | 124 | trigger the qualcomm-case-agent SKILL through an external agent CLI (claude/cline/gemini), for the dashboard's manual "Sync now" button. |
...
| `tests/pipeline.test.mjs` | 313 | Unit tests for the headless pipeline's pure logic + a dashboard smoke test. |
