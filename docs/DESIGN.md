...
> Source fingerprint `52148b29e314` over 81 files.
...
| `tests/qcomm_cdp_client.test.mjs` | 283 | Unit tests for Native WebSocket CDP Client (Slice 1). |
...
| `tests/qcomm_dom_helpers.test.mjs` | 209 | Loads dom_helpers.js's exact source into a jsdom context (script execution, not a hand-copied re-implementation) and exercises each shared helper — including the shadow-DOM case behind case 08503838 that motivated this file (see dom_helpers.js). |
...
| `tests/qcomm_extract_case.test.mjs` | 1833 | — |
| `tests/qcomm_fast_landing.test.mjs` | 1231 | — |
| `tests/qcomm_finalize_case.test.mjs` | 1284 | Tests for the finalizer — the module that decides what gets persisted. |
| `tests/qcomm_intake.test.mjs` | 121 | QA coverage for intake.mjs — the skill's INPUT CONTRACT gate. |
| `tests/qcomm_login_fill.test.mjs` | 138 | — |
| `tests/qcomm_pipeline.test.mjs` | 181 | Unit tests for the headless pipeline's pure logic. |
| `tests/qcomm_render_case.test.mjs` | 712 | QA coverage for render_case.mjs Tests that render_case.mjs generates ONLY case.md (clean markdown) with Header metadata, Initial Description, and Chronological Timeline of comments, without generating HTML, PDF, txt, or report.md files. |
| `tests/qcomm_run_case.test.mjs` | 896 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
| `tests/qcomm_screenshot_timeout.test.mjs` | 39 | Regression test for bounded screenshot() timeout. |
