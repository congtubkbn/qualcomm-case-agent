...
> Source fingerprint `b8148ff23d79` over 57 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/delete_case.mjs` | 112 | permanent, agent-confirmed removal of one case's local cache. |
...
| `.claude/skills/qualcomm-case-agent/scripts/extract_case.js` | 365 | Default case extractor for PHASE 2. |
...
| `.claude/skills/qualcomm-case-agent/scripts/render_case.mjs` | 77 | deterministic markdown renderer for the Qualcomm Case Management Agent. |
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 581 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 818 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `tests/delete_case.test.mjs` | 101 | Tests for delete_case.mjs — permanent, agent-confirmed case cache removal. |
| `tests/description_first_comment.test.mjs` | 374 | Comprehensive test suite for PRD #53 / Issues #54, #55, #56: Case Description as Initial Comment across Extraction, Ingestion, Merge, Rendering, and Migration. |
| `tests/e2e_landing_smoke.mjs` | 135 | E2E Smoke Test for Fast CDP Landing Engine. |
| `tests/extract_case.test.mjs` | 1073 | — |
| `tests/fast_landing.test.mjs` | 544 | — |
| `tests/intake.test.mjs` | 121 | QA coverage for intake.mjs — the skill's INPUT CONTRACT gate. |
| `tests/migrate_case.test.mjs` | 310 | — |
| `tests/migrate_case_detail.test.mjs` | 211 | — |
...
| `tests/render_case.test.mjs` | 257 | QA coverage for render_case.mjs Tests that render_case.mjs generates ONLY case.md (clean markdown) with Header metadata, Initial Description, and Chronological Timeline of comments, without generating HTML, PDF, txt, or report.md files. |
| `tests/run_case.test.mjs` | 551 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
| `tests/scrape_case.test.mjs` | 802 | Tests for the finalizer — the module that decides what gets persisted. |
...
| `tools/migrate_case.mjs` | 200 | Re-sort comments chronologically, re-classify roles, sanitize comment schema, re-calculate case hash, and re-render case.md for cached cases. |
| `tools/migrate_case_detail.mjs` | 289 | Upgrade and migrate cached cases with Salesforce Detail tab metadata, re-render case.md with Detail fields, and update case index/overview. |
...
| `npm run case:delete` | `node .claude/skills/qualcomm-case-agent/scripts/delete_case.mjs` |
