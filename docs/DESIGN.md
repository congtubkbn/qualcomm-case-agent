...
> Source fingerprint `cb5921535504` over 35 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/extract_case.js` | 166 | Default case extractor for PHASE 2. |
...
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 503 | the whole capture pipeline as ONE deterministic command. |
...
| `tests/extract_case.test.mjs` | 365 | — |
...
| `tests/render_case.test.mjs` | 284 | QA coverage for render_case.mjs — previously untested. |
| `tests/run_case.test.mjs` | 257 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
| `tests/scrape_case.test.mjs` | 489 | Tests for the finalizer — the module that decides what gets persisted. |
