...
> Source fingerprint `056cf1d3a10a` over 26 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 395 | the whole capture pipeline as ONE deterministic command. |
...
| `tests/run_case.test.mjs` | 162 | Tests for run_case.mjs's browser-driving state machine (landOnCase / findCaseLink) — the part of the pipeline that decides whether we actually landed on the real case page or a Lightning stub. |
| `tests/scrape_case.test.mjs` | 346 | Tests for the finalizer — the module that decides what gets persisted. |
