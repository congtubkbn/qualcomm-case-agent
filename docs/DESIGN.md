...
> Source fingerprint `260527aa3009` over 62 files.
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-agent/scripts/_paths.mjs` | 75 | single source of truth for skill paths (Node / ESM). |
| `.claude/skills/qualcomm-case-agent/scripts/_paths.ps1` | 83 | single source of truth for skill paths (PowerShell). |
| `.claude/skills/qualcomm-case-agent/scripts/browser.mjs` | 276 | thin Node wrapper around the `agent-browser` CLI. |
...
| `.claude/skills/qualcomm-case-agent/scripts/check_collapsed.js` | 71 | Pure read: how many non-anchor posts still end in the "Expand Post" label right now. |
...
| `.claude/skills/qualcomm-case-agent/scripts/delete_case.mjs` | 141 | permanent, agent-confirmed removal of one case's local cache. |
| `.claude/skills/qualcomm-case-agent/scripts/dom_helpers.js` | 90 | Shared DOM helpers for page scripts (expand_step.js, check_collapsed.js, switch_tab.js, login_fill.js). |
| `.claude/skills/qualcomm-case-agent/scripts/expand_step.js` | 159 | PHASE 1.5 (A and B) as ONE browser-side tick, called in a loop from run_case.mjs. |
...
| `.claude/skills/qualcomm-case-agent/scripts/login_fill.js` | 148 | In-page script that drives Okta username & password fill steps and classifies the outcome: AUTHENTICATED, OTP_REQUIRED, REJECTED, or UNKNOWN. |
...
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 679 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 1002 | Persistence post-processor for the AGENT-DRIVEN extraction. |
| `.claude/skills/qualcomm-case-agent/scripts/secret_store.mjs` | 51 | DPAPI secret store for Qualcomm ID password (issue #126). |
| `.claude/skills/qualcomm-case-agent/scripts/switch_tab.js` | 99 | Switches active tab on Salesforce Lightning case page (e.g., between "Detail" and "Feed"). |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-summary/scripts/run_summary.mjs` | 225 | orchestrator for qualcomm-case-summary. |
...
| `run_summary.mjs` | `computeDelta(caseComments, summarizedIds)` | function | delta.mjs inline |
| `run_summary.mjs` | `captureCase(code)` | async fn | deps.mjs inline |
| `run_summary.mjs` | `mergeSummary(prior, { caseNumber, title, url, priority, product, status, newComments, flow, executive, now })` | function | merge.mjs inline |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-overview/scripts/cases_overview.mjs` | 174 | CLI orchestrator: wires the overview store, dashboard renderer, and CLI table renderer together into the `cases_overview` command. |
| `.claude/skills/qualcomm-case-overview/scripts/cli_renderer.mjs` | 61 | Renders the terminal summary table for Qualcomm case overview data. |
| `.claude/skills/qualcomm-case-overview/scripts/dashboard_renderer.mjs` | 1548 | Renders the self-contained, offline HTML dashboard for Qualcomm case overview data. |
| `.claude/skills/qualcomm-case-overview/scripts/overview_store.mjs` | 342 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |

Exported API — overview scripts:

| Module | Export | Kind | Contract |
|---|---|---|---|
| `cases_overview.mjs` | `afterFinalize(caseCode, dataDir)` | function | Syncs the cases overview and dashboard after a case finalizes (capture or summary). |
...
| `dashboard_renderer.mjs` | `getStatusCategory(status)` | function | Categorizes status for badge colors and filtering. |
...
| `tests/cases_overview_after_finalize.test.mjs` | 98 | Tests for cases_overview.mjs's afterFinalize — the shared overview/dashboard sync seam both scrape_case.mjs and run_summary.mjs call after a case finalizes. |
...
| `tests/cases_overview_e2e.test.mjs` | 319 | End-to-end integration tests for cases_overview: 1. |
| `tests/cases_overview_render.test.mjs` | 468 | Unit and integration tests for cases_overview rendering (HTML Dashboard & CLI table). |
...
| `tests/credentials_setup.test.mjs` | 76 | — |
| `tests/delete_case.test.mjs` | 181 | Tests for delete_case.mjs — permanent, agent-confirmed case cache removal. |
...
| `tests/dom_helpers.test.mjs` | 128 | Loads dom_helpers.js's exact source into a jsdom context (script execution, not a hand-copied re-implementation) and exercises each shared helper — including the shadow-DOM case behind case 08503838 that motivated this file (see dom_helpers.js). |
...
| `tests/extract_case.test.mjs` | 1833 | — |
...
| `tests/qualcomm_case_summary_metadata.test.mjs` | 188 | Tests for qualcomm-case-summary's case-level metadata header + executive summary block. |
| `tests/qualcomm_case_summary_orchestrator.test.mjs` | 278 | Tests for qualcomm-case-summary's orchestrator (prepare/finalize). |
...
| `tests/scrape_case.test.mjs` | 1105 | Tests for the finalizer — the module that decides what gets persisted. |
...
| `npm run setup:credentials` | `powershell -ExecutionPolicy Bypass -File .claude/skills/qualcomm-case-agent/scripts/setup/capture_credentials.ps1` |
