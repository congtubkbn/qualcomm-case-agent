...
> Source fingerprint `012d69ab7912` over 86 files.
...
| `.claude/skills/qcomm/scripts/finalize_case.mjs` | 1039 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `.claude/skills/qcomm/scripts/overview_store.mjs` | 446 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
...
| `tests/qcomm_finalize_case.test.mjs` | 1338 | Tests for the finalizer — the module that decides what gets persisted. |
