...
| `finalize_case.mjs` | Thin orchestrator: composes the `finalize_*` policy modules (completeness, identity/hash, merge, normalize, header) into canonical write + index update | Browser, analysis, owning any one policy directly |
...
> Source fingerprint `abab238b0257` over 83 files.
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qcomm/scripts/_paths.mjs` | 90 | single source of truth for skill paths (Node / ESM). |
...
| `.claude/skills/qcomm/scripts/cases_overview.mjs` | 165 | CLI orchestrator: wires the overview store, dashboard renderer, and CLI table renderer together into the `cases_overview` command. |
| `.claude/skills/qcomm/scripts/cdp_client.mjs` | 470 | Deep Module: native WebSocket CDP client for Chrome DevTools Protocol. |
| `.claude/skills/qcomm/scripts/cdp_portal_driver.mjs` | 371 | Production PortalDriver: drives the persistent-profile Chrome over CDP. |
| `.claude/skills/qcomm/scripts/cli_renderer.mjs` | 63 | Renders the terminal summary table for Qualcomm case overview data. |
| `.claude/skills/qcomm/scripts/comment_tree.mjs` | 125 | Sole owner of the Comment Tree shape: the nested `subs:[]` structure that case.json persists (a Comment's replies live in its own `subs` array, not a separate structure — see CONTEXT.md's Comment Tree entry). |
...
| `.claude/skills/qcomm/scripts/dashboard_client_script.mjs` | 426 | Embedded client-side script for the offline Qualcomm case dashboard (filtering, searching, theme toggle, auto-refresh, modal interactions). |
| `.claude/skills/qcomm/scripts/dashboard_renderer.mjs` | 193 | Assembles the self-contained, offline HTML dashboard for Qualcomm case overview data from its styles, client script, and row/page templating modules. |
| `.claude/skills/qcomm/scripts/dashboard_row.mjs` | 150 | Renders a single case's summary/detail row pair for the dashboard table. |
| `.claude/skills/qcomm/scripts/dashboard_styles.mjs` | 805 | Embedded CSS stylesheet for the offline Qualcomm case dashboard. |
| `.claude/skills/qcomm/scripts/delete_case.mjs` | 131 | Permanent, agent-confirmed removal of one case's local cache. |
| `.claude/skills/qcomm/scripts/detail_fields.mjs` | 119 | Sole owner of the Detail Field Provenance policy (CONTEXT.md, ADR 0008): deciding, per Detail-tab field, whether a fresh portal value should overwrite the cached case.json or the cached value should win. |
| `.claude/skills/qcomm/scripts/dom_extractor.js` | 1050 | Unified, namespaced DOM extractor and browser automation helper module. |
| `.claude/skills/qcomm/scripts/ensure_protocol.mjs` | 174 | Core Self-Healing Protocol Engine for Qualcomm Case Agent (`qc://`). |
| `.claude/skills/qcomm/scripts/fast_landing.mjs` | 418 | — |
| `.claude/skills/qcomm/scripts/finalize_case.mjs` | 373 | Persistence post-processor for the AGENT-DRIVEN extraction. |
| `.claude/skills/qcomm/scripts/finalize_completeness.mjs` | 66 | Completeness gate policy: fail-closed checks that stop finalize() from persisting a capture that is short, still collapsed, or otherwise less than the portal actually shows — a failed capture must never look like a successful "no update". |
| `.claude/skills/qcomm/scripts/finalize_description.mjs` | 104 | Description-as-first-comment policy (PRD #53 / Issues #54-56): synthesizes the Case's description field into a presentation comment when the portal feed does not already carry it, and derives the 1-2 sentence preview summary persisted on every comment. |
| `.claude/skills/qcomm/scripts/finalize_header.mjs` | 36 | Header-flag policy: the fields the agent already holds in-context from the PHASE 1 search row (title/status/priority/severity), passed to finalize_case.mjs as CLI flags so the big raw file never has to be re-Read just to add a title (O(1) tokens, not O(case size)), plus the Detail-tab metadata field list. |
| `.claude/skills/qcomm/scripts/finalize_identity.mjs` | 86 | Comment identity + hash policy: content-derived comment ids (stable across runs regardless of feed position), legacy-id migration, and the case-level completeness hash finalize_case.mjs stamps onto every persisted case.json. |
| `.claude/skills/qcomm/scripts/finalize_merge.mjs` | 74 | Comment merge policy: union fresh comments into a cache by content id, enforce chronological order, and flag same-author high-similarity "new" comments as possible edits of an old one for a human to check. |
| `.claude/skills/qcomm/scripts/finalize_normalize.mjs` | 230 | Timestamp and author-role normalization policy: Chatter relative timestamps ("13h ago") resolved to absolute ISO-8601 against a capture reference date, chronological ordering with a display-position tie-break, and author role classification (Qualcomm / Customer / System) used by the renderer. |
| `.claude/skills/qcomm/scripts/fixture_portal_driver.mjs` | 121 | Offline PortalDriver: replays pre-recorded snapshots instead of driving Chrome. |
| `.claude/skills/qcomm/scripts/html_escape.mjs` | 17 | Generic HTML-escaping utility shared across dashboard templating modules. |
| `.claude/skills/qcomm/scripts/intake.mjs` | 61 | Intake guard: validate case code + prep cache dirs. |
| `.claude/skills/qcomm/scripts/lock.mjs` | 87 | one capture at a time, machine-wide. |
| `.claude/skills/qcomm/scripts/open_qc_case.mjs` | 319 | Deep Module: Core URI Parser & CDP / Chrome Dispatcher for qc:// custom protocol scheme. |
| `.claude/skills/qcomm/scripts/overview_lock.mjs` | 91 | Process lock module guarding _overview.json atomic read-modify-write-rename operations. |
| `.claude/skills/qcomm/scripts/overview_store.mjs` | 367 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
| `.claude/skills/qcomm/scripts/portal_driver.mjs` | 88 | The seam between run_case.mjs (orchestration: verdicts, retries, screenshots-per-failure-mode) and however a case actually gets read off the portal. |
...
| `.claude/skills/qcomm/scripts/register_protocol.ps1` | 62 | Registers the custom URL protocol scheme (qc://) in the Windows Registry for the current user. |
| `.claude/skills/qcomm/scripts/render_case.mjs` | 200 | Deterministic markdown renderer for the Qualcomm Case Management Agent. |
| `.claude/skills/qcomm/scripts/repeat_until_stable.mjs` | 51 | CDP-agnostic polling primitive (#275): ticks a caller-supplied function until its result stops changing, or gives up after maxRounds. |
| `.claude/skills/qcomm/scripts/run_case.mjs` | 522 | The whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qcomm/scripts/secret_store.mjs` | 47 | DPAPI secret store for Qualcomm ID password (issue #126). |
...
| `.claude/skills/qcomm/scripts/status_category.mjs` | 28 | Status classification shared by dashboard row rendering and page-level counts. |
...
| `.claude/skills/qcomm/scripts/verify_case.mjs` | 186 | Post-capture QA gate for run_case.mjs's output. |
...
| `cases_overview.mjs` | `openInBrowser(filePath)` | function |  |
| `cases_overview.mjs` | `parseArgs(args)` | function |  |
| `cases_overview.mjs` | `rebuildOverview(casesDir = DEFAULT_CASES_DIR)` | function |  |
| `cdp_client.mjs` | `CdpError` | value |  |
| `cdp_client.mjs` | `CdpClient` | value |  |
| `cdp_portal_driver.mjs` | `FEED_PROBE_ROUNDS` | value |  |
| `cdp_portal_driver.mjs` | `EXPAND_ROUNDS` | value | slower than page readiness right after a fresh login (cold Lightning component bootstrap) |
| `cdp_portal_driver.mjs` | `STUCK_RETRY_ROUNDS` | value |  |
| `cdp_portal_driver.mjs` | `SETTLE_ROUNDS` | value | while still clicking "Expand Post" every tick |
| `cdp_portal_driver.mjs` | `POST_EXPAND_SETTLE_ROUNDS` | value |  |
| `cdp_portal_driver.mjs` | `DETAIL_SWITCH_RETRIES` | value | "Expand Post" to get a real click and settle before extraction |
| `cdp_portal_driver.mjs` | `CdpPortalDriver` | value |  |
| `cli_renderer.mjs` | `renderCliTable(overviewData, options = {})` | function |  |
| `comment_tree.mjs` | `flattenComments(comments)` | function | Flatten a nested tree (case.json's subs:[] shape) back to a flat array with parentId re-attached. |
| `comment_tree.mjs` | `buildNestedTree(comments)` | function | Build a nested tree from a flat list that already has parentId set. |
| `comment_tree.mjs` | `countAllComments(comments)` | function |  |
| `comment_tree.mjs` | `findCommentNode(nodes, id)` | function |  |
| `comment_tree.mjs` | `cloneCommentTree(nodes)` | function | Deep-clones a nested tree (any depth), so a caller can mutate the copy (e.g. |
| `comment_tree.mjs` | `walkCommentTree(comments, visitor, prefix = '', depth = 0, parent = null)` | function | Walks a nested comment tree (with subs: []) and yields a flat list of comment entries decorated with hierarchical numbers (e.g. |
| `dashboard_client_script.mjs` | `renderClientScript()` | function |  |
| `dashboard_renderer.mjs` | `renderHeader(stats)` | function |  |
| `dashboard_renderer.mjs` | `renderControlsBar(stats, counts)` | function |  |
| `dashboard_renderer.mjs` | `renderCasesTable(rowsHtml)` | function |  |
| `dashboard_renderer.mjs` | `renderProtocolModal()` | function |  |
| `dashboard_renderer.mjs` | `renderDashboardHtml(overviewData, outputPath = null)` | function |  |
| `dashboard_row.mjs` | `renderCaseRow(c)` | function |  |
| `dashboard_styles.mjs` | `renderStyles()` | function |  |
...
| `detail_fields.mjs` | `MERGE_FIELDS` | value | Detail-tab metadata fields merged into `raw` every run (see mergeDetailFields below) and applied to the cache on an update run (see applyDetailFieldOverrides). |
| `detail_fields.mjs` | `DRIFT_CHECK_FIELDS` | value | Fields the fast no-update probe checks for drift (see detailFieldsDiffer). |
| `detail_fields.mjs` | `detailFieldsDiffer(detailRaw, cached, fields)` | function | The fast no-update probe short-circuits BEFORE finalize() ever runs (no write, no detailChanged plumbing — cdp_portal_driver.mjs returns noUpdate straight from the probe). |
| `detail_fields.mjs` | `mergeDetailFields(raw, detailRaw, fields)` | function | Detail tab is the only tab that actually renders `fields`; Feed-tab `raw` can still stumble onto a non-empty value for them (wrong DOM region), so detailRaw wins whenever present, and raw is the fallback only when detailRaw didn't capture the field. |
| `detail_fields.mjs` | `applyDetailFieldOverrides(out, raw, fields)` | function | Persist-time half of the policy: applies `raw`'s Detail Field Provenance (`raw.detailFields`, set by mergeDetailFields above) onto the cached `out` object finalize_case.mjs's merge branch is building. |
| `ensure_protocol.mjs` | `isProtocolRegistered(options = {})` | function |  |
| `ensure_protocol.mjs` | `ensureProtocolRegistered(options = {})` | function | If missing on Windows, silently registers it via register_protocol.ps1. |
...
| `finalize_case.mjs` | `EXIT` | value |  |
| `finalize_case.mjs` | `finalize(caseCode, rawPath, header = {}, merge = false, options = {})` | function |  |
...
| `finalize_completeness.mjs` | `countAssert(capturedCount, displayedCount)` | function | Completeness gate comparison (Issue #91): The Salesforce Chatter badge ("N Chatter Feed Items") counts only top-level posts, whereas our Feed extractor captures both top-level posts AND nested replies (e.g. |
| `finalize_completeness.mjs` | `genuineCommentCount(comments, description)` | function | The synthesized description comment is a presentation convenience derived from the Case's description field, not a captured Chatter feed item — the completeness gate must compare against genuine portal comments only. |
| `finalize_description.mjs` | `extractSummary(body)` | function |  |
| `finalize_description.mjs` | `synthesizeDescriptionComment(raw)` | function |  |
| `finalize_description.mjs` | `hasDescriptionComment(comments, description)` | function |  |
| `finalize_header.mjs` | `HEADER_KEYS` | value |  |
| `finalize_header.mjs` | `DETAIL_KEYS` | value | Salesforce Lightning Detail tab metadata fields persisted to canonical case.json. |
| `finalize_header.mjs` | `parseHeaderFlags(argv)` | function | Parse ` title "..."` style flags into an overrides object. |
| `finalize_identity.mjs` | `computeHash(raw)` | function | Hash only the raw, verbatim fields. |
| `finalize_identity.mjs` | `commentKey(c)` | function | Stable identity for dedup across runs. |
| `finalize_identity.mjs` | `commentId(c)` | function | Content-derived comment id: the same comment gets the same id in every run, whatever position it now occupies in the feed. |
| `finalize_identity.mjs` | `assignIds(comments)` | function | Assign content ids to a comment list. |
| `finalize_identity.mjs` | `migrateIds(cached)` | function | Bring a cached case written with the old positional ids (c1, c2, …) or with raw DOM ids onto content ids. |
| `finalize_merge.mjs` | `mergeComments(cachedComments, rawComments, referenceDate = new Date())` | function | Merge raw comments not already cached and enforce chronological sorting (Oldest -> Newest). |
| `finalize_normalize.mjs` | `isBlacklistedTs(s)` | function |  |
| `finalize_normalize.mjs` | `classifyRole(author, company = '', context = '', body = '')` | function |  |
| `finalize_normalize.mjs` | `parseTimestamp(ts, referenceDate = new Date())` | function |  |
| `finalize_normalize.mjs` | `normalizeComment(comment, referenceDate = new Date())` | function |  |
| `finalize_normalize.mjs` | `normalizeComments(comments, referenceDate = new Date())` | function |  |
| `finalize_normalize.mjs` | `sortCommentsChronological(comments, referenceDate = new Date())` | function | Sorts comments strictly in chronological order (Oldest -> Newest). |
| `fixture_portal_driver.mjs` | `DEFAULT_FIXTURE_DIR` | value |  |
| `fixture_portal_driver.mjs` | `FixturePortalDriver` | value |  |
| `html_escape.mjs` | `escapeHtml(str)` | function | Escapes HTML characters in string to prevent XSS. |
...
| `open_qc_case.mjs` | `isValidQualcommUrl(urlString)` | function |  |
| `open_qc_case.mjs` | `parseQcUri(uriString)` | function | Supported patterns: - qc://case/<caseNumber> - qc://<caseNumber> - qc:case/<caseNumber> - qc:<caseNumber> - qc://open?url=<encoded-url> - qc:open?url=<encoded-url> |
...
| `open_qc_case.mjs` | `resolveTargetUrl(parsed, options = {})` | function | Reads data/cases/<caseNumber>/case.json if available; falls back to Qualcomm global search. |
| `open_qc_case.mjs` | `dispatchQcTarget(targetUrl, options = {})` | async fn |  |
| `open_qc_case.mjs` | `openQcCase(rawUri, options = {})` | async fn |  |
| `overview_lock.mjs` | `acquireOverviewLock(casesDir, opts = {})` | function | Returns a { path, token } handle to pass to releaseOverviewLock. |
| `overview_lock.mjs` | `releaseOverviewLock(lock)` | function | Safe no-op if lock was already reclaimed or taken over by another process token. |
| `overview_lock.mjs` | `withOverviewLock(casesDir, fn, opts = {})` | function |  |
| `overview_store.mjs` | `DEFAULT_CASES_DIR` | value |  |
| `overview_store.mjs` | `createCommentSnippet(comment, maxLen = 160)` | function |  |
| `overview_store.mjs` | `extractProductFromTitle(title)` | function | Example: "[DE.3.1.4][SM7635] epsfb_if_emc_and_no_vonr does not work" -> "SM7635" |
| `overview_store.mjs` | `extractCaseOverview(caseDir, caseNumber = '')` | function |  |
| `overview_store.mjs` | `computeStats(cases)` | function |  |
| `overview_store.mjs` | `buildOverviewData(casesDir = DEFAULT_CASES_DIR)` | function |  |
| `overview_store.mjs` | `syncCaseOverview(caseNumber, options = {})` | function |  |
| `overview_store.mjs` | `updateCaseOverview(caseNumber, casesDir = DEFAULT_CASES_DIR)` | function | Backwards-compatible wrapper delegating to syncCaseOverview with render: false. |
| `overview_store.mjs` | `applyFilter(overviewData, filter)` | function |  |
| `portal_driver.mjs` | `PortalDriver` | value |  |
| `render_case.mjs` | `formatBody(body)` | function |  |
| `render_case.mjs` | `generateMarkdown(data, stem = 'case')` | function |  |
| `render_case.mjs` | `renderCase(jsonPath)` | function |  |
| `repeat_until_stable.mjs` | `repeatUntilStable({)` | async fn | the current tick's value against the previous tick's value. |
...
| `run_case.mjs` | `formatVerdict(code, v = {}, started)` | function |  |
...
| `secret_store.mjs` | `readPassword(secretPath = SECRET_PATH)` | function |  |
| `secret_store.mjs` | `clearSecret(secretPath = SECRET_PATH)` | function |  |
...
| `status_category.mjs` | `getStatusCategory(status, ballInCourt)` | function | Categorizes status for badge colors and filtering. |
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 977 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
| `tests/qcomm_chronological_sort.test.mjs` | 318 | Unit tests for chronological comment timestamp parsing and Oldest -> Newest sorting. |
| `tests/qcomm_comment_tree.test.mjs` | 249 | Tests for comment_tree.mjs — the sole owner of the Comment Tree shape (case.json's nested subs:[] structure). |
...
| `tests/qcomm_detail_fields.test.mjs` | 176 | Tests for detail_fields.mjs — the sole owner of the Detail Field Provenance policy (CONTEXT.md, ADR 0008). |
| `tests/qcomm_dom_helpers.test.mjs` | 231 | Loads dom_extractor.js's exact source into a jsdom context (script execution, not a hand-copied re-implementation) and exercises each shared helper — including the shadow-DOM case behind case 08503838 that motivated this file (see dom_extractor.js). |
...
| `tests/qcomm_finalize_case.test.mjs` | 1277 | Tests for the finalizer — the module that decides what gets persisted. |
| `tests/qcomm_finalize_normalize.test.mjs` | 57 | Characterization tests for finalize_normalize.mjs's classifyRole and isBlacklistedTs — split out of finalize_case.mjs (#250) with no prior isolated coverage (only exercised indirectly, if at all, through render_case.mjs / normalizeComment). |
...
| `tests/qcomm_login_fill.test.mjs` | 169 | — |
| `tests/qcomm_offline_cli_contract.test.mjs` | 149 | Offline CLI contract suite (#241): exercises all 3 qcomm CLI entry points — run_case, delete_case, cases_overview — as real child processes (real stdout, real exit code), each against an isolated QUALCOMM_ROOT and FixturePortalDriver (via QCOMM_FIXTURE_DIR, wired in run_case.mjs for #241). |
| `tests/qcomm_overview_data.test.mjs` | 809 | Unit and integration tests for cases_overview data scanning and aggregation engine. |
| `tests/qcomm_overview_e2e.test.mjs` | 203 | End-to-end integration tests for cases_overview: 1. |
...
| `tests/qcomm_overview_render.test.mjs` | 674 | Unit and integration tests for cases_overview rendering (HTML Dashboard & CLI table). |
...
| `tests/qcomm_render_case.test.mjs` | 790 | QA coverage for render_case.mjs Tests that render_case.mjs generates ONLY case.md (clean markdown) with Header metadata, Initial Description, and Chronological Timeline of comments, without generating HTML, PDF, txt, or report.md files. |
| `tests/qcomm_repeat_until_stable.test.mjs` | 104 | Unit tests for the CDP-agnostic repeatUntilStable polling primitive (#275). |
| `tests/qcomm_run_case.test.mjs` | 1197 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
...
| `tests/qcomm_verify_case.test.mjs` | 128 | Tests for the post-capture QA gate. |
...
| File | Lines | Purpose |
|---|---|---|
| `tools/gen_design.mjs` | 221 | regenerate the machine-derived part of docs/DESIGN.md. |
...
| V2 | A short or empty capture never overwrites a good cached case | `finalize_case.mjs` gates: 0 comments, `finalize_completeness.mjs`'s `countAssert`, title gate |
| V3 | `finalize_case.mjs` is the only writer of raw fields and `hash` | `finalize_identity.mjs`'s `computeHash` covers verbatim fields only; no other module touches `case.json` |
...
| V10 | Comment identity is content-derived, so merge/dedup never re-attaches state to the wrong comment | `finalize_identity.mjs`'s `commentId` / `assignIds`; `migrateIds` for legacy caches; unit tests |
| V11 | Overview sync and dashboard rendering errors never block capture or delete verdicts | `syncCaseOverview` try-catch error isolation + stderr warning logging |
