...
> Source fingerprint `70313bdc2b32` over 34 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/browser.mjs` | 210 | thin Node wrapper around the `agent-browser` CLI. |
| `.claude/skills/qualcomm-case-agent/scripts/cdp_client.mjs` | 318 | Deep Module: Native WebSocket CDP client for Chrome DevTools Protocol. |
...
| `.claude/skills/qualcomm-case-agent/scripts/fast_landing.mjs` | 379 | Deep Module: Direct Nav + Event-Driven Search & Landing Engine. |
...
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 490 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| File | Lines | Purpose |
|---|---|---|
| `web/cli_run.mjs` | 127 | trigger the qualcomm-case-agent SKILL through an external agent CLI (claude/cline/gemini), for the dashboard's manual "Sync now" button. |
| `web/server.mjs` | 208 | local dashboard for the Qualcomm case cache. |

Exported API — dashboard:

| Module | Export | Kind | Contract |
|---|---|---|---|
| `cli_run.mjs` | `CONFIG_PATH` | value |  |
| `cli_run.mjs` | `loadCliConfig()` | function |  |
| `cli_run.mjs` | `fillArgv(template, prompt)` | function | Substitute the {prompt} token into an argv template. |
| `cli_run.mjs` | `classifyRun(before, after, cliFailed, reason)` | function | Derive a verdict purely from disk state before/after the CLI ran — never from the agent's own reply text. |

#### Tests

| File | Lines | Purpose |
|---|---|---|
| `tests/cdp_client.test.mjs` | 146 | Unit tests for Native WebSocket CDP Client (Slice 1). |
| `tests/e2e_landing_smoke.mjs` | 135 | E2E Smoke Test for Fast CDP Landing Engine. |
| `tests/fast_landing.test.mjs` | 434 | — |
| `tests/intake.test.mjs` | 121 | QA coverage for intake.mjs — the skill's INPUT CONTRACT gate. |
| `tests/pipeline.test.mjs` | 320 | Unit tests for the headless pipeline's pure logic + a dashboard smoke test. |
| `tests/render_case.test.mjs` | 201 | QA coverage for render_case.mjs — previously untested. |
| `tests/run_case.test.mjs` | 310 | Tests for run_case.mjs's browser-driving state machine (landOnCase / findCaseLink) — the part of the pipeline that decides whether we actually landed on the real case page or a Lightning stub. |
| `tests/scrape_case.test.mjs` | 452 | Tests for the finalizer — the module that decides what gets persisted. |
...
| Command | Runs |
|---|---|
| `npm run test` | `node --experimental-test-module-mocks --test tests/pipeline.test.mjs tests/scrape_case.test.mjs tests/run_case.test.mjs tests/cdp_client.test.mjs tests/fast_landing.test.mjs` |
