...
```jsonc
{
  ...
  "comments": [                            // OLDEST FIRST, verbatim, never truncated
...
> Source fingerprint `6cc183f3185b` over 62 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/extract_case.js` | 470 | Default case extractor for PHASE 2. |
...
| `.claude/skills/qualcomm-case-agent/scripts/render_case.mjs` | 208 | deterministic markdown renderer for the Qualcomm Case Management Agent. |
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 645 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 968 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `.claude/skills/qualcomm-case-agent/scripts/verify_case.mjs` | 158 | post-capture QA gate for run_case.mjs's output. |
...
| `.claude/skills/qualcomm-case-summary/scripts/merge.mjs` | 30 | pure merge of a new batch of comment summaries into summary.json's shape. |
| `.claude/skills/qualcomm-case-summary/scripts/render_summary.mjs` | 55 | pure summary.md renderer for qualcomm-case-summary. |
...
| `tests/cases_overview_data.test.mjs` | 540 | Unit and integration tests for cases_overview data scanning and aggregation engine. |
...
| `tests/description_first_comment.test.mjs` | 403 | Comprehensive test suite for PRD #53 / Issues #54, #55, #56: Case Description as Initial Comment across Extraction, Ingestion, Merge, Rendering, and Migration. |
...
| `tests/extract_case.test.mjs` | 1716 | — |
...
| `tests/qualcomm_case_summary_render.test.mjs` | 148 | Tests for qualcomm-case-summary's summary.md renderer (pure, no mocking needed). |
...
| `tests/scrape_case.test.mjs` | 1020 | Tests for the finalizer — the module that decides what gets persisted. |
