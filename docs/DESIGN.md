...
> Source fingerprint `150663ad6bb4` over 79 files.
...
| `.claude/skills/qcomm/scripts/browser.mjs` | 270 | thin Node wrapper around the `agent-browser` CLI. |
...
| `.claude/skills/qcomm/scripts/dom_extractor.js` | 1020 | Unified, namespaced DOM extractor and browser automation helper module. |
| `.claude/skills/qcomm/scripts/ensure_protocol.mjs` | 180 | Core Self-Healing Protocol Engine for Qualcomm Case Agent (`qc://`). |
| `.claude/skills/qcomm/scripts/fast_landing.mjs` | 425 | — |
...
| `tests/qcomm_dom_helpers.test.mjs` | 210 | Loads dom_helpers.js's exact source into a jsdom context (script execution, not a hand-copied re-implementation) and exercises each shared helper — including the shadow-DOM case behind case 08503838 that motivated this file (see dom_helpers.js). |
...
| `tests/qcomm_fast_landing.test.mjs` | 1206 | — |
