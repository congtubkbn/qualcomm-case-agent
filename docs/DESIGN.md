...
> Source fingerprint `80d4befe85e2` over 75 files.
...
| `.claude/skills/qcomm/scripts/comment_tree.mjs` | 132 | Sole owner of the Comment Tree shape: the nested `subs:[]` structure that case.json and summary.json both persist (a Comment's replies live in its own `subs` array, not a separate structure — see CONTEXT.md's Comment Tree entry). |
...
| `.claude/skills/qcomm/scripts/overview_store.mjs` | 444 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
...
| `.claude/skills/qcomm/scripts/run_case.mjs` | 495 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qcomm/scripts/run_summary.mjs` | 378 | orchestrator for qcomm summary. |
...
| `run_summary.mjs` | `computeDelta(caseComments, summarizedIds)` | function | delta.mjs inline A reply the top-level filter used to miss is still checked against summarizedIds, since flattenTreeComments walks every depth. |
...
| `tests/qcomm_comment_tree.test.mjs` | 249 | Tests for comment_tree.mjs — the sole owner of the Comment Tree shape (case.json/summary.json's nested subs:[] structure). |
...
| `tests/qcomm_offline_cli_contract.test.mjs` | 158 | Offline CLI contract suite (#241): exercises all 4 qcomm CLI entry points — run_case, run_summary, delete_case, cases_overview — as real child processes (real stdout, real exit code), each against an isolated QUALCOMM_ROOT and FixturePortalDriver (via QCOMM_FIXTURE_DIR, wired in run_case.mjs for #241). |
...
| `tests/qcomm_overview_e2e.test.mjs` | 320 | End-to-end integration tests for cases_overview: 1. |
...
| `tests/qcomm_summary_merge.test.mjs` | 340 | Tests for qcomm's merge logic (pure, no mocking needed). |
