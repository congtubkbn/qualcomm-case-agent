...
> Source fingerprint `87c7fc865391` over 81 files.
...
| `.claude/skills/qcomm/scripts/cdp_portal_driver.mjs` | 333 | Production PortalDriver: drives the persistent-profile Chrome over CDP. |
...
| `.claude/skills/qcomm/scripts/open_qc_case.mjs` | 349 | Deep Module: Core URI Parser & CDP / Chrome Dispatcher for qc:// custom protocol scheme. |
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 868 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
