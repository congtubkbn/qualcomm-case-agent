...
> Source fingerprint `5f92157e0283` over 59 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/extract_case.js` | 435 | Default case extractor for PHASE 2. |
...
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 581 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 963 | Persistence post-processor for the AGENT-DRIVEN extraction. |
| `.claude/skills/qualcomm-case-agent/scripts/switch_tab.js` | 70 | Switches active tab on Salesforce Lightning case page (e.g., between "Detail" and "Feed"). |
...
| `tests/extract_case.test.mjs` | 1577 | — |
...
| `tests/run_case.test.mjs` | 552 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
