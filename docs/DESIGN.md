...
> Source fingerprint `89afe9dee9e3` over 75 files.
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 150 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
