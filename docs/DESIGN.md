...
> Source fingerprint `6457bb047cfe` over 75 files.
...
| `.claude/skills/qcomm/scripts/dashboard_renderer.mjs` | 1627 | Renders the self-contained, offline HTML dashboard for Qualcomm case overview data. |
...
| `.claude/skills/qcomm/scripts/finalize_case.mjs` | 355 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `.claude/skills/qcomm/scripts/overview_store.mjs` | 416 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
...
| `finalize_normalize.mjs` | `isRelativeTimestamp(ts)` | function | Checks if a timestamp string is a relative Chatter format (e.g. |
...
| `overview_store.mjs` | `afterFinalize(caseCode, dataDir, options = {})` | function | Resilient synchronization hook for a finalized capture. |
...
| `tests/qcomm_chronological_sort.test.mjs` | 329 | Unit tests for chronological comment timestamp parsing and Oldest -> Newest sorting. |
...
| `tests/qcomm_finalize_normalize.test.mjs` | 62 | Characterization tests for finalize_normalize.mjs's classifyRole and isBlacklistedTs — split out of finalize_case.mjs (#250) with no prior isolated coverage (only exercised indirectly, if at all, through render_case.mjs / normalizeComment). |
