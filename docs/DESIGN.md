...
> Source fingerprint `cd78a0d5c8d3` over 79 files.
...
| `.claude/skills/qcomm/scripts/finalize_case.mjs` | 400 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `.claude/skills/qcomm/scripts/run_case.mjs` | 599 | the whole capture pipeline as ONE deterministic command. |
...
| `run_case.mjs` | `detailFieldsDiffer(detailRaw, cached, fields)` | function | The fast no-update probe short-circuits BEFORE finalize() ever runs (no write, no detailChanged plumbing — cdp_portal_driver.mjs returns noUpdate straight from the probe). |
...
| `tests/qcomm_finalize_case.test.mjs` | 1347 | Tests for the finalizer — the module that decides what gets persisted. |
...
| `tests/qcomm_run_case.test.mjs` | 1235 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
