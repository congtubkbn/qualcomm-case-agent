...
> Source fingerprint `2ce7f60d4688` over 75 files.
...
| `.claude/skills/qcomm/scripts/dashboard_renderer.mjs` | 1619 | Renders the self-contained, offline HTML dashboard for Qualcomm case overview data. |
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 118 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
