...
> Source fingerprint `5af53a4b043e` over 82 files.
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qcomm/scripts/_paths.mjs` | 75 | single source of truth for skill paths (Node / ESM). |
| `.claude/skills/qcomm/scripts/_paths.ps1` | 83 | single source of truth for skill paths (PowerShell). |
| `.claude/skills/qcomm/scripts/browser.mjs` | 251 | thin Node wrapper around the `agent-browser` CLI. |
| `.claude/skills/qcomm/scripts/cases_overview.mjs` | 164 | CLI orchestrator: wires the overview store, dashboard renderer, and CLI table renderer together into the `cases_overview` command. |
| `.claude/skills/qcomm/scripts/cdp_client.mjs` | 443 | Deep Module: Native WebSocket CDP client for Chrome DevTools Protocol. |
| `.claude/skills/qcomm/scripts/check_collapsed.js` | 54 | Pure read: how many non-anchor posts still end in the "Expand Post" label right now. |
...
| `.claude/skills/qcomm/scripts/dashboard_renderer.mjs` | 1582 | Renders the self-contained, offline HTML dashboard for Qualcomm case overview data. |
| `.claude/skills/qcomm/scripts/delete_case.mjs` | 131 | permanent, agent-confirmed removal of one case's local cache. |
| `.claude/skills/qcomm/scripts/dom_helpers.js` | 138 | Shared DOM helpers for page scripts (expand_step.js, check_collapsed.js, switch_tab.js, login_fill.js). |
| `.claude/skills/qcomm/scripts/expand_step.js` | 151 | PHASE 1.5 (A and B) as ONE browser-side tick, called in a loop from run_case.mjs. |
| `.claude/skills/qcomm/scripts/extract_case.js` | 477 | Default case extractor for PHASE 2. |
| `.claude/skills/qcomm/scripts/fast_landing.mjs` | 423 | — |
| `.claude/skills/qcomm/scripts/finalize_case.mjs` | 1001 | Persistence post-processor for the AGENT-DRIVEN extraction. |
| `.claude/skills/qcomm/scripts/intake.mjs` | 62 | Intake guard: validate case code + prep cache dirs. |
| `.claude/skills/qcomm/scripts/lock.mjs` | 79 | one capture at a time, machine-wide. |
| `.claude/skills/qcomm/scripts/login_fill.js` | 148 | In-page script that drives Okta username & password fill steps and classifies the outcome: AUTHENTICATED, OTP_REQUIRED, REJECTED, or UNKNOWN. |
| `.claude/skills/qcomm/scripts/observe_case_state.js` | 70 | In-page script that evaluates page state or uses MutationObserver to wait for ON_CASE / AUTH after a navigation or click. |
...
| `.claude/skills/qcomm/scripts/overview_store.mjs` | 441 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
...
| `.claude/skills/qcomm/scripts/render_case.mjs` | 210 | deterministic markdown renderer for the Qualcomm Case Management Agent. |
| `.claude/skills/qcomm/scripts/run_case.mjs` | 678 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qcomm/scripts/run_summary.mjs` | 261 | orchestrator for qcomm summary. |
| `.claude/skills/qcomm/scripts/search_case_results.js` | 139 | In-page script that waits for global-search results to render and extracts the matching case link plus whatever fields (title/status/priority/severity) can be read off the result row. |
...
| `.claude/skills/qcomm/scripts/switch_tab.js` | 99 | Switches active tab on Salesforce Lightning case page (e.g., between "Detail" and "Feed"). |
| `.claude/skills/qcomm/scripts/verify_case.mjs` | 160 | post-capture QA gate for run_case.mjs's output. |
...
| `tests/qcomm_paths.test.mjs` | 63 | Unit tests for _paths.mjs's project-root resolution, and a regression guard that every consumer of "the cases directory" resolves through the same root as _paths.mjs (a worktree checkout must never fork its own data/cases — see _paths.mjs comment). |
