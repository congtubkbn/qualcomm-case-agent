...
> Source fingerprint `28b28721d82d` over 81 files.
...
| `tests/qcomm_summary_delta.test.mjs` | 45 | Tests for qcomm's delta computation (pure, no mocking needed). |
| `tests/qcomm_summary_merge.test.mjs` | 152 | Tests for qcomm's merge logic (pure, no mocking needed). |
| `tests/qcomm_summary_metadata.test.mjs` | 188 | Tests for qcomm's case-level metadata header + executive summary block. |
| `tests/qcomm_summary_orchestrator.test.mjs` | 356 | Tests for qcomm's orchestrator (prepare/finalize). |
| `tests/qcomm_summary_render.test.mjs` | 149 | Tests for qcomm's summary.md renderer (pure, no mocking needed). |
