...
> Source fingerprint `ac3d4e8e60ce` over 60 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/browser.mjs` | 268 | thin Node wrapper around the `agent-browser` CLI. |
...
| `.claude/skills/qualcomm-case-agent/scripts/extract_case.js` | 439 | Default case extractor for PHASE 2. |
...
| `.claude/skills/qualcomm-case-agent/scripts/render_case.mjs` | 211 | deterministic markdown renderer for the Qualcomm Case Management Agent. |
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 619 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 963 | Persistence post-processor for the AGENT-DRIVEN extraction. |
| `.claude/skills/qualcomm-case-agent/scripts/switch_tab.js` | 108 | Switches active tab on Salesforce Lightning case page (e.g., between "Detail" and "Feed"). |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-overview/scripts/cases_overview.mjs` | 1899 | Aggregation engine, CLI summary table, and offline HTML dashboard for Qualcomm cases. |
...
| `tests/ensure_protocol.test.mjs` | 115 | Unit and integration tests for scripts/ensure_protocol.mjs (Self-Healing Protocol Engine). |
| `tests/extract_case.test.mjs` | 1656 | — |
...
| `tests/open_qc_case.test.mjs` | 280 | Unit & integration tests for the qc:// custom protocol dispatcher. |
...
| `tests/qualcomm_case_summary_metadata.test.mjs` | 187 | Tests for qualcomm-case-summary's case-level metadata header + executive summary block. |
...
| `tests/run_case.test.mjs` | 684 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
