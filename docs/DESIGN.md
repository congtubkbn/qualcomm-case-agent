...
> Source fingerprint `63540b2ec435` over 34 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/extract_case.js` | 266 | Default case extractor for PHASE 2. |
...
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 528 | the whole capture pipeline as ONE deterministic command. |
...
| `tests/extract_case.test.mjs` | 551 | — |
| `tests/fast_landing.test.mjs` | 544 | — |
| `tests/intake.test.mjs` | 121 | QA coverage for intake.mjs — the skill's INPUT CONTRACT gate. |
| `tests/migrate_case.test.mjs` | 234 | — |
| `tests/pipeline.test.mjs` | 135 | Unit tests for the headless pipeline's pure logic + a dashboard smoke test. |
| `tests/render_case.test.mjs` | 219 | QA coverage for render_case.mjs Tests that render_case.mjs generates ONLY case.md (clean markdown) with Header metadata, Initial Description, and Chronological Timeline of comments, without generating HTML, PDF, txt, or report.md files. |
| `tests/run_case.test.mjs` | 425 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
| `tests/scrape_case.test.mjs` | 490 | Tests for the finalizer — the module that decides what gets persisted. |
...
| `tools/migrate_case.mjs` | 243 | Re-sort comments chronologically, re-classify roles, sanitize comment schema, re-calculate case hash, and re-render case.md for cached cases. |

Exported API — doc tooling:

| Module | Export | Kind | Contract |
|---|---|---|---|
| `migrate_case.mjs` | `isBlacklistedTs(s)` | function |  |
| `migrate_case.mjs` | `extractSummary(body)` | function |  |
