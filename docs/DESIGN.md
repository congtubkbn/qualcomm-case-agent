...
> Source fingerprint `c5c42c8c955b` over 75 files.
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 745 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
