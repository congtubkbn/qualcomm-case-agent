...
> Source fingerprint `e978105cbafb` over 40 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/scrape_case.mjs` | 737 | Persistence post-processor for the AGENT-DRIVEN extraction. |
...
| `tests/qualcomm_case_summary_orchestrator.test.mjs` | 257 | Tests for qualcomm-case-summary's orchestrator (prepare/finalize). |
| `tests/qualcomm_case_summary_render.test.mjs` | 49 | Tests for qualcomm-case-summary's summary.md renderer (pure, no mocking needed). |
...
| `tests/scrape_case.test.mjs` | 568 | Tests for the finalizer — the module that decides what gets persisted. |
