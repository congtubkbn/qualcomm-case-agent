...
> Source fingerprint `2ba92d618828` over 82 files.
...
| `.claude/skills/qcomm/scripts/run_summary.mjs` | 300 | orchestrator for qcomm summary. |
...
| `tests/qcomm_summary_merge.test.mjs` | 210 | Tests for qcomm's merge logic (pure, no mocking needed). |
| `tests/qcomm_summary_metadata.test.mjs` | 188 | Tests for qcomm's case-level metadata header + executive summary block. |
| `tests/qcomm_summary_orchestrator.test.mjs` | 371 | Tests for qcomm's orchestrator (prepare/finalize). |
