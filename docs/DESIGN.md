...
> Source fingerprint `41e889363426` over 75 files.
...
| `tests/qcomm_cdp_portal_driver.test.mjs` | 501 | Tests for CdpPortalDriver.connect()'s neutral-descriptor contract (issue: fix "Classify connection verdicts through the PortalDriver seam, not browser.mjs's error classes"). |
