...
> Source fingerprint `1706747be763` over 85 files.
...
| `.claude/skills/qualcomm-case-agent/scripts/capture_case.mjs` | 29 | captureCase(code): spawns run_case.mjs as a subprocess and parses its one-line JSON verdict. |
...
| `.claude/skills/qualcomm-case-agent/scripts/fast_landing.mjs` | 423 | — |
...
| `.claude/skills/qualcomm-case-agent/scripts/observe_case_state.js` | 70 | In-page script that evaluates page state or uses MutationObserver to wait for ON_CASE / AUTH after a navigation or click. |
...
| `.claude/skills/qualcomm-case-agent/scripts/run_case.mjs` | 678 | the whole capture pipeline as ONE deterministic command. |
| `.claude/skills/qualcomm-case-agent/scripts/search_case_results.js` | 139 | In-page script that waits for global-search results to render and extracts the matching case link plus whatever fields (title/status/priority/severity) can be read off the result row. |
...
| `capture_case.mjs` | `captureCase(code)` | async fn |  |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-summary/scripts/run_summary.mjs` | 240 | orchestrator for qualcomm-case-summary. |
...
| File | Lines | Purpose |
|---|---|---|
| `.claude/skills/qualcomm-case-overview/scripts/cases_overview.mjs` | 164 | CLI orchestrator: wires the overview store, dashboard renderer, and CLI table renderer together into the `cases_overview` command. |
...
| `.claude/skills/qualcomm-case-overview/scripts/overview_lock.mjs` | 95 | Process lock module guarding _overview.json atomic read-modify-write-rename operations. |
| `.claude/skills/qualcomm-case-overview/scripts/overview_store.mjs` | 457 | Data/domain module: scans case directories, shapes overview records, computes stats, and persists the aggregated _overview.json atomically. |
...
| `tests/cases_overview_lock.test.mjs` | 162 | Tests for overview_store.mjs's write lock around _overview.json (issue #202): concurrent syncCaseOverview calls for different cases must not last-write-wins clobber each other, and the lock itself must take over a stale holder and give up after a bounded wait instead of hanging forever. |
...
| `tests/finalize_case.test.mjs` | 1284 | Tests for the finalizer — the module that decides what gets persisted. |
...
| `tests/merge_case_json.test.mjs` | 189 | Tests for the case.json union merge driver (issue #210, spec #205). |
...
| `tests/qualcomm_case_summary_orchestrator.test.mjs` | 342 | Tests for qualcomm-case-summary's orchestrator (prepare/finalize). |
...
| `tests/run_case.test.mjs` | 896 | Tests for run_case.mjs's pipeline orchestrator, anchor logic, and stuck detection. |
...
| `tools/merge_case_json.mjs` | 89 | Custom git merge driver for data/cases/**/case.json (issue #210, spec #205). |
| `tools/regen_case_markdown.mjs` | 36 | Redraws case.md from case.json for every case directory under cwd. |
...
| `merge_case_json.mjs` | `mergeCaseJson(oursText, theirsText)` | function | Pure: text in, text out. |
| `regen_case_markdown.mjs` | `regenAllCaseMarkdown(root = process.cwd())` | function |  |
...
| `npm run postinstall` | `node scripts/ensure_protocol.mjs && node scripts/ensure_merge_driver.mjs` |
