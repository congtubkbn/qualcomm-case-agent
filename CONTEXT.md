...
as downstream capabilities — deletes a case's local cache and aggregates cached cases into an
overview dashboard (`qcomm`).
...
`case.json`, `case.md`, comments) and its `_index.json` entry. Irreversible. Owned by
