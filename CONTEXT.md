...
Local tool that captures Qualcomm Support cases into a local cache (`qcomm`), and —
as a downstream capability — reports their status and summarizes their comments for an
engineer reviewing a case (`qcomm`).
...
comment) produced by `qcomm`. Distinct from the comment's own verbatim body.
...
or where it's stuck), maintained by `qcomm` and updated incrementally as new
comments arrive.
_Avoid_: enrichment.caseFlow (the deleted field from ADR 0001 — this is a new, separately-owned
concept, not its revival)

**Delta (comments)**:
The set of a case's comments not yet present in a prior `qcomm` run, computed by
comment-id difference, never by array position or count. Drives summarizing only what's new.

**Hide (case)**:
A per-browser, client-side-only flag on the `qcomm` dashboard that removes a case
...
`qcomm` (the data owner), never by `qcomm`'s read-only overview dashboard.
